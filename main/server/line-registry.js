...
function validateLine(input) {
    ...
    return { lineId, name, site, dashboardUrl, cardSize };
}

function fromRow(row) {
    return { lineId: row.line_id, name: row.display_name, site: row.site, dashboardUrl: row.dashboard_url || '', cardSize: row.card_size || 'standard', sortOrder: Number(row.sort_order) || 0 };
}

function createLineRegistry({ pool, hasDatabaseConfig }) {
    const configs = new Map(DEFAULT_LINES.map((line, index) => [line.lineId, { ...line, dashboardUrl: line.dashboardUrl || '', cardSize: 'standard', sortOrder: index }]));
    ...
    function ensureLoaded() {
        ...
        if (!loadPromise) {
            loadPromise = (async () => {
                ...
                const result = await pool.query('SELECT line_id, display_name, site, dashboard_url, card_size, sort_order FROM production_overview_lines WHERE deleted = FALSE ORDER BY site, sort_order, line_id');
                configs.clear();
    ...
    async function create(input) {
        ...
        await pool.query(`
            INSERT INTO production_overview_lines (line_id, display_name, site, dashboard_url, card_size, sort_order, deleted)
            ...
            ON CONFLICT (line_id) DO UPDATE SET
              ...
              deleted = FALSE
        `, [line.lineId, line.name, line.site, line.dashboardUrl, line.cardSize, line.sortOrder]);
    ...
    async function update(lineId, input) {
        ...
        await pool.query(`
            UPDATE production_overview_lines SET display_name = $1, site = $2, dashboard_url = $3, card_size = $4, sort_order = $5
            WHERE line_id = $6
        `, [line.name, line.site, line.dashboardUrl, line.cardSize, line.sortOrder, lineId]);
